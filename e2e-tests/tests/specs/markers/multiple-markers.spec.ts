...
describe("Google Maps - Multiple Markers", function () {
  ...
  it("should create a map and add 4 markers", async function () {
    ...
    const commandOutput = await $(
      (
        await MultipleMarkers.commandOutputTextarea
      ).selector
    ).$("textarea");
  ...
  it("should enable and disable clustering", async function () {
    ...
    const disableClusteringButton =
      await MultipleMarkers.disableClusteringButton;
    const commandOutput = await $(
      (
        await MultipleMarkers.commandOutputTextarea
      ).selector
  ...
  it("should throw error when attempting to remove no markers", async function () {
    ...
    await removeMarkersButton.tap();
    await pause(500);
    await expect(commandOutput).toHaveValue(
      "Invalid Arguments Provided: markerIds requires at least one marker id."
    );
  });
});
