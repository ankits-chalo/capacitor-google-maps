...
describe("Google Maps - Marker Customization", function () {
  ...
  it("should create a map and add 1 color marker", async function () {
    ...
    const removeAllMarkersButton =
      await MarkerCustomizations.removeAllMarkersButton;
    const addColorMarkerButton =
      await MarkerCustomizations.addMarkerColorButton;
  ...
  it("should create a map and add 1 image marker", async function () {
    ...
    const addImageMarkerButton =
      await MarkerCustomizations.addMarkerImageButton;
    const commandOutput = await $(
      (
        await MarkerCustomizations.commandOutputTextarea
      ).selector
  ...
  it("should add 4 image markers", async function () {
    const removeAllMarkersButton =
      await MarkerCustomizations.removeAllMarkersButton;
    const addMultipleImageMarkersButton =
      await MarkerCustomizations.addMultipleImageMarkersButton;
    const commandOutput = await $(
      (
        await MarkerCustomizations.commandOutputTextarea
      ).selector
