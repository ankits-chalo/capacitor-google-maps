...
describe("Google Maps - Add and Remove Marker", function () {
  ...
  it("should create a map and add a marker", async function () {
    ...
    const markerId = (await getCommandOutputText()).replace(
      "Marker added: ",
      ""
    );
  ...
  it("should remove the created marker", async function () {
    ...
    const commandOutput = await $(
      (
        await AddAndRemoveMarkers.commandOutputTextarea
      ).selector
    ).$("textarea");

    await removeMarkerButton.tap();
    await pause(500);
    await expect(commandOutput).toHaveValueContaining(
      `Marker removed: ${createdMarkerId}`
    );
  });
  ...
  it("should throw when attempting to remove a non-existent marker", async function () {
    ...
    await pause(500);
    await expect(commandOutput).toHaveValueContaining(
      `Marker not found for provided id.`
    );
  });
});
